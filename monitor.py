...
class Monitor:
    ...
    def _average_ride_distance(self):
        ...
        total_distance = 0
        for activities in self._activities[RIDER].values():
            # satisfied rider's activities must be in [REQUEST, PICKUP, DROPOFF, CANCEL] order
            # or in [REQUEST, PICKUP, CANCEL, DROPOFF] order
            if activities[-1].id == DROPOFF:
                total_distance += manhattan_distance(activities[0].location, activities[-1].location)
            else:
                total_distance += manhattan_distance(activities[0].location, activities[2].location)
        return total_distance / len(self._activities[DRIVER])
